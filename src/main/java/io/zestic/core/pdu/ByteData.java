...
/**
 * Base class for all object which can be transformed to sequence of bytes and which can be re-read from sequence of
 * bytes.
 */
...
public abstract class ByteData implements Serializable {
    ...
    /**
     * Defines a format of part of the smpp defined date format which is
     * parseable by Java SimpleDateFormat parser. The rest (nnp) is parsed
     * 'manually'.
     */
    ...
    /**
     * Controls checking of the date-time format in the library. If this
     * variable to is set to <code>true</code> the library will check if the
     * date is correctly formated according SMPP spec; if it's
     * <code>false</code>, then the date will be sent without checking in the
     * library and the check will be done by the SMSC. Default is
     * <code>true</code>. Note that whatever is the setting, the library will
     * still check the length of the date-time string.
     */
    private static boolean libraryCheckDateFormat = true;

    /**
     * Static initialiser initialises the <code>dateFormatter</code> with format
     * specified for SMPP Date/Time format and sets other formatter parameters.
     */
    static {
    ...
    /**
     * This abstract method should parse the buffer with binary data passed as
     * parameter into member variables.
     *
     * @param buffer the data which should contain binary representation of the
     *               class
     * @throws PduException                       some data in the buffer were invalid
     * @throws NotEnoughDataInByteBufferException expected more data in the
     *                                            buffer
     * @throws TerminatingZeroNotFoundException   the c-string in buffer wasn't
     *                                            terminated with 0 zero
     * @see #getData()
     */
     ...
     * @throws ValueNotSetException thrown from a TLV if the value was requested
     *                              but never set
    ...
    /**
     * Default constructor. Only this is present as this abstract class doesn't
    ...
    /**
     * Checks if the length of the data of the string is less or equal than the
     * provided maximum; necessery for multibyte encodings. Note that the length
     * checked is the length wfter transforming the string to series of octets,
     * so two-byte strings will efectively require space (max size) two-times
    ...
    /**
     * Checks if the length of string is greater or equal than provided minimum
     * and less or equal than the provided maximum.
     *
     * @param string the string to check
     * @param min    the minimal length of the string
     * @param max    the maximal length of the string
     * @throws WrongLengthOfStringException thrown if the string is shorter than
     *                                      min length or longer than max length
     */
    ...
    /**
     * Checks if the length of the data of the string is greater or equal than
     * provided minimum and less or equal than the provided maximum; necessery
    ...
    /**
     * Checks if the length of the string plus 1 for terminating zero is less or
     * equal than provided maximum.
     *
     * @param string the string to check
    ...
    /**
     * Checks if the length of the string plus 1 for terminating zero is greater
     * or equal than provided minimum and less or equal than provided maximum.
     *
     * @param string the string to check
     * @param min    the minimal length of the string with added term. zero
     * @param max    the maximal length of the string with added term. zero
     * @throws WrongLengthOfStringException thrown if string with added
     *                                      terminating zero would be shorter than minimum or longer than the maximum
     */
    protected static void checkCString(String string, int min, int max)
            throws WrongLengthOfStringException {
        int count = string == null ? 1 : (string.length() + 1); // 1 is for
        // terminating
        // zero
    ...
    /**
     * Checks if the string contains valid date string as specified in SMPP
     * spec.
     *
     * @param dateStr the date to check
     * @throws WrongDateFormatException throwsn if the string doesn't contain
     *                                  date with valid format
     */
    protected static void checkDate(String dateStr)
            throws WrongDateFormatException {
        int count = dateStr == null ? 1 : (dateStr.length() + 1); // 1 is for
        // terminating
        // zero
        if ((count != 1) && (count != Constants.DATE_LEN)) {
        ...
        if ("+-R".lastIndexOf(locTime) == -1) {
            ...
            throw new WrongDateFormatException(dateStr,
                    "time difference relation indicator incorrect; "
    ...
    /**
     * Allow a variable of type <code>byte</code> to carry lengths or sizes up to 255. The integral types are signed in
     * Java, so if there is necessary to store an unsigned type into signed of the same size, the value can be stored
     * as negative number even if it would be positive in unsigned case. For example message length can be 0 to 254
     * and is carried by 1 octet i.e. unsigned 8 bits. We use a byte variable to read the value from octet stream.
     * If the length is >127, it is interpreted as negative byte using negative complement notation. So length 150
     * would be interpreted as Java byte -106. If we want to know the actual value (*length) we need to make a
     * correction to a bigger integral type, in case of byte it's short. The correction from (negative) byte to short
     * is<br> <code>(short)(256+(short)length)</code><br> This converts the negative byte value representing positive
     * length into positive short value.
     *
    ...
    /**
     * Provides correction of positive unsigned 2 byte carried by (signed)
     * <code>short</code> into positive signed <code>int</code>. See explanation
     * in <code>decodeUnsigned(byte)</code>.
     *
